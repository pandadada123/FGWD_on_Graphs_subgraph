...
inline long long selectRootFromQuery() {
	...
	// Determine if all nodes have core number less than 2
	bool allNodesHaveLowCore = true;
	for (long long i = 0; i < g_cnt_node_query_graph; i++) {
		if (g_core_number_query_graph[i] >= 2) {
			allNodesHaveLowCore = false;
			break;
		}
	}
	...
	for (long long i = 0; i < g_cnt_node_query_graph; i++) {
		...
		// If not all nodes have low core and current node has core number less than 2
		// skip this iteration (unless all nodes have low core)
		if (!allNodesHaveLowCore && g_core_number_query_graph[i] < 2) {
		...
		// node labels are greater or equal than 1
		// if (label == 0) {  
			// ranking = 0.0;
		// }
		// else {
			...
			cout << g_label_to_sort_by_degree_node_data_graph[label - 1].first << endl;
			cout << g_label_to_sort_by_degree_node_data_graph[label - 1].second << endl;
		...
		// Ensuring nodes with a ranking of 0 are not considered as start_node
		if (ranking != 0.0 && ranking < least_ranking) {
