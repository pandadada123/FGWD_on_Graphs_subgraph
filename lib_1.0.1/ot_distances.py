           ...
           for i in range(n):
               for ii in range(n):
                   ...
                   if i==ii:
                       ...
                       try:
                           test = nx.shortest_path_length(G.nx_graph,source=key1,target=key2) # test whether the node is isolated
                           paths = nx.all_simple_paths(G.nx_graph, source=key1, target=key2)
                           lengths =[]
                           for path in paths:
