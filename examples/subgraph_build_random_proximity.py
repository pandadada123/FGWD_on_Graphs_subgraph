...
import copy
from ot_distances_proximity import Prox
...
#%% Proximity
        # Cost
        # Gc = emd(a, b, Cost)
        str_metric2 = 'proximity'
        # dprox, log_prox, transp_prox, M, C = Prox(
        #     alpha=alpha, features_metric=fea_metric, method=str_metric2, loss_fun='square_loss').graph_d(G1, G2, p1, p2, p2_nodummy)
        transp_prox, M, C = Prox(
            alpha=alpha, features_metric=fea_metric, method=str_metric2, loss_fun='square_loss').graph_d(G1, G2, p1, p2, p2_nodummy)
        # %% FGWD, find alpha
        # alld=[]
