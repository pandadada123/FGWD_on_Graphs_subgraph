...
sys.path.append(os.path.realpath(
    'E:/Master Thesis/FGWD_on_Graphs_subgraph/lib_1.0.2'))
...
N2 = 50 # additional nodes in large graph
# NN2 =[5,10,20,40,60]
# NN2=[20]
...
def build_G1(G, N2=30, numfea=3, pw=0.5):
    ...
    for i in range(NN):
        for j in range(i+1, NN):
            ...
            # no additional edge within the subgraph
...
for numfea in NumFea:
    ...
    while num < Num:
        ...
        dfgw, log_FGWD, transp_FGWD, M, C1, C2 = Fused_Gromov_Wasserstein_distance(
            alpha=alpha, features_metric=fea_metric, method='shortest_path', loss_fun='square_loss').graph_d(G1, G2, p1, p2, p2_nodummy)
        fig = plt.figure(figsize=(10, 8))
...
plt.fill_between(np.array(NumFea), np.array(
    Mean)-np.array(STD), np.array(Mean)+np.array(STD))
# %% plot percentage
plt.figure()
